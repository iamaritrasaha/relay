...
This audit covers tracked product sources containing `Relay`, `relay`,
`org.relay`, or `relay_app`, with generated Dart output treated as a
...
means Relay interoperability or legal attribution.
...
| In-app visible diagnostics and documents | user-facing Relay wording in `app/lib/**`, `README.md`, and current-release documentation | Replace product wording with Relay; preserve actual Relay compatibility wording under C and attribution under B. |
...
## B. Legal / attribution: keep Relay
...
## C. Interoperability: keep Relay name

| Location/pattern | Why it stays |
| --- | --- |
| `/api/relay/...` routes, v2/v3 DTO/client/server comments and tests in `packages/core/src/http/**` | Wire-compatible Relay protocol path; changing it breaks LAN/manual interoperability. |
| `RelayPeer`, `LegacyLanInboundSession`, Relay protocol/discovery comments in `packages/core/src/relay/**`, `packages/core/src/multicast/**`, and `packages/core/src/discovery/**` | Explicitly distinguishes unauthenticated Relay-compatible LAN peers from Relay-authenticated peers. |
| Relay certificate/fingerprint compatibility wording in `packages/core/src/crypto/**` and `packages/core/src/http/client/**` | Documents protocol behavior, not Relay product presentation. |
| Browser/share wording that specifically means a Relay peer or protocol endpoint | Keep only where the meaning is interoperability; otherwise change the visible product wording under A. |

## D. Internal identifier / migration-sensitive: do not rename automatically

| Item | Representative locations | Migration impact |
| --- | --- | --- |
| Android application/package ID `com.foresight.app.relay` | `app/android/app/build.gradle`, manifests, Kotlin package directory, method-channel constants | Changing it breaks Android upgrade continuity and installed-app identity; it also changes deep links, permissions, signing/update lineage, and channel lookup. |
...
| Flutter Dart package `relay_app` | `app/pubspec.yaml` and all `package:relay_app/...` imports | Broad build/import migration; does not itself change protocol compatibility but requires a coordinated package rename. |
| Dart isolate package `relay_isolates` and Rust plugin/crate `rust_lib_relay_app` | `packages/relay_isolates/**`, cargokit/build files, generated FRB bindings | Build/package/import migration with high generated-code and platform-artifact risk; no automatic rename. |
...
| Preference/database/storage namespaces containing `relay` | persistence helpers, platform paths, secure-store identifiers, and any serialized namespace discovered during RA4C planning | Can strand persisted preferences/history/database data unless old namespaces are read and migrated. |
