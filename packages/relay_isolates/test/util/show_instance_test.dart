// Regression test for the upstream single-instance handoff behaviour.
//
// A share-target invocation starts a second process with the file paths as
...
import 'package:relay_isolates/rust/api/crypto.dart' hide SecurityContext;
...
File _findDylib() {
  ...
  return [
    File('$base/librust_lib_relay_app.dylib'),
    File('$base/librust_lib_relay_app.so'),
    File('$base/rust_lib_relay_app.dll'),
  ].firstWhere((f) => f.existsSync(), orElse: () => File('$base/librust_lib_relay_app.dylib'));
}
...
void main() {
  ...
  test('hands the arguments to the running instance over TLS', () async {
    if (!_findDylib().existsSync()) {
      markTestSkipped('Rust dylib not built (cargo build -p rust_lib_relay_app)');
      return;
    }
