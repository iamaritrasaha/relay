//! Production-shaped mutual inner TLS.
//!
//! Uses the same certificate validation machinery as LAN HTTP/TLS:
//! [`crate::http::client::PinnedServerCertVerifier`] and
//! [`crate::http::server::common::client_cert_verifier::CustomClientCertVerifier`].
//! Peer Relay identity remains authoritative only after `RelayIdentityProofV1`.
...
use crate::http::server::common::client_cert_verifier::CustomClientCertVerifier;

/// Independent inner-TLS identity for one endpoint.
///
        ...
        let client_config = rustls::ClientConfig::builder()
            ...
            .with_custom_certificate_verifier(Arc::new(
                PinnedServerCertVerifier::try_new(&cert.certificate_pem, None)
                    .context("build production inner TLS client verifier")?,
            ))
        ...
        let server_config = rustls::ServerConfig::builder()
            .with_client_cert_verifier(Arc::new(
                CustomClientCertVerifier::try_new(&cert.certificate_pem, true)
                    .context("build production inner TLS server verifier")?,
            ))
