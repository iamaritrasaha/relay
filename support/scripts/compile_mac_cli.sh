...
ARM_NAME="Relay-CLI-$VERSION-macos-arm-64"
X64_NAME="Relay-CLI-$VERSION-macos-x86-64"
...
echo
cargo build --release --package relay-cli --target aarch64-apple-darwin
cargo build --release --package relay-cli --target x86_64-apple-darwin
...
cp target/aarch64-apple-darwin/release/relay-cli "$ARM_NAME/relay-cli"
cp target/x86_64-apple-darwin/release/relay-cli "$X64_NAME/relay-cli"
...
codesign --force --verbose --options runtime --sign "$SIGN_ID" "$ARM_NAME/relay-cli"
codesign --force --verbose --options runtime --sign "$SIGN_ID" "$X64_NAME/relay-cli"
...
echo
zip -j "$ARM_NAME.zip" "$ARM_NAME/relay-cli"
zip -j "$X64_NAME.zip" "$X64_NAME/relay-cli"
...
echo
tar -czvf "$ARM_NAME.tar.gz" -C "$ARM_NAME" relay-cli
tar -czvf "$X64_NAME.tar.gz" -C "$X64_NAME" relay-cli
