...
Relay là một ứng dụng mã nguồn mở miễn phí cho phép bạn chia sẻ tệp và tin nhắn một cách an toàn với các thiết bị lân cận qua mạng cục bộ mà không cần kết nối Internet.
...
Relay là một ứng dụng đa nền tảng cho phép liên lạc an toàn giữa các thiết bị bằng cách sử dụng API REST và mã hóa HTTPS. Không giống như các ứng dụng nhắn tin khác dựa trên máy chủ bên ngoài, Relay không yêu cầu kết nối Internet hoặc máy chủ của bên thứ ba, khiến ứng dụng này trở thành giải pháp nhanh chóng và đáng tin cậy cho liên lạc nội bộ.
...
| Android  | 5.0                 | -                                                                                                                           |
| iOS      | 12.0                | -                                                                                                                           |
| macOS    | 11 Big Sur          | Sử dụng OpenCore Legacy Patcher 2.0.2 (Xem [#1005](https://github.com/relay/relay/issues/1005#issuecomment-2449899384)) |
...
Trong hầu hết các trường hợp, Relay sẽ hoạt động tốt. Tuy nhiên, nếu gặp sự cố khi gửi hoặc nhận tệp, bạn có thể cần định cấu hình tường lửa để cho phép Relay liên lạc qua mạng cục bộ của mình.
...
Để khởi động ứng dụng ẩn (chỉ trong khay), hãy sử dụng cờ `--hidden` (ví dụ: `relay.exe --hidden`).

Trên v1.14.0 trở về trước, ứng dụng sẽ bắt đầu ẩn nếu cờ `autostart` được đặt và cài đặt ẩn được bật.

## Nó hoạt động như thế nào

Relay sử dụng giao thức liên lạc an toàn cho phép các thiết bị liên lạc với nhau bằng API REST. Tất cả dữ liệu được gửi an toàn qua HTTPS và chứng chỉ TLS/SSL được tạo nhanh chóng trên mỗi thiết bị, đảm bảo tính bảo mật tối đa.

Để biết thêm thông tin về Giao thức Relay, hãy xem phần [tài liệu](https://github.com/relay/protocol).

## Bắt đầu

Để biên dịch Relay từ mã nguồn, hãy làm theo các bước sau:

1.  Cài đặt Flutter [trực tiếp](https://flutter.dev)hoặc sử dụng [fvm](https://fvm.app) (xem [phiên bản cần thiết](.fvmrc))
2.  Sao chép `Relay` kho lưu trữ
...
> [!GHI CHÚ] Relay hiện yêu cầu phiên bản Flutter cũ hơn (được chỉ định trong [.fvmrc](.fvmrc))
> và do đó, các sự cố xây dựng có thể xảy ra do sự không khớp giữa phiên bản Flutter được cài đặt và yêu cầu (toàn hệ thống).  
> Để phát triển nhất quán hơn, Relay sử dụng [fvm](https://fvm.app)để quản lý phiên bản Flutter của dự án.
> Sau khi cài đặt `fvm`, chạy `fvm flutter` thay vì `flutter`.

## Đóng góp

Chúng tôi hoan nghênh sự đóng góp từ bất kỳ ai quan tâm đến việc giúp cải thiện Relay. Nếu bạn muốn đóng góp, có một số cách để tham gia:

### Dịch thuật

Bạn có thể giúp dịch Relay sang các ngôn ngữ khác! **Cách được khuyến nghị**: Sử dụng platform [Weblate](https://hosted.weblate.org/projects/relay/app) để quản lý bản dịch.

**Cách khác**: Bạn cũng có thể đóng góp bằng cách fork repository này và thêm bản dịch theo cách thủ công.

Bản dịch nằm trong thư mục [app/assets/i18n](https://github.com/relay/relay/tree/main/app/assets/i18n). Chỉnh sửa tệp `_missing_translations_<locale>.json` hoặc `strings_<locale>.i18n.json` để thêm hoặc cập nhật bản dịch.

<a href="https://hosted.weblate.org/engage/relay/">
<img src="https://hosted.weblate.org/widget/relay/app/multi-auto.svg" alt="Trạng thái dịch" />
</a>
...
- **Cải tiến:** Bạn có ý tưởng về cách cải thiện Relay? Vui lòng tạo một vấn đề trước để thảo luận lý do tại sao cần cải thiện.

Để biết thêm thông tin, xem [hướng dẫn đóng góp](https://github.com/relay/relay/blob/main/CONTRIBUTING.md).
