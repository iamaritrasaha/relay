...
Relay는 인터넷에 연결하지 않고도 로컬 네트워크를 통해 주변 기기들과 파일 및 메시지를 안전하게 공유할 수 있는 무료 오픈소스 앱입니다.
...
Relay는 REST API 및 HTTPS 암호화를 통해 기기 간의 파일을 안전하게 주고 받을 수 있는 크로스플랫폼 애플리케이션입니다. 외부 서버에 의존하는 다른 앱과 달리, Relay는 인터넷 연결 및 제삼자 서버를 필요로 하지 않는 만큼, 근거리 통신을 위한 빠르고 안정적인 대안이라고 할 수 있습니다.
...
| Android  | 5.0             | -                                                                                                                            |
| iOS      | 12.0            | -                                                                                                                            |
| macOS    | 11 Big Sur      | OpenCore Legacy Patcher 2.0.2 필요 ([#1005](https://github.com/relay/relay/issues/1005#issuecomment-2449899384) 참고)|
| Windows  | 10              | Windows 7을 지원하는 마지막 버전은 v1.15.4입니다. 추후 Windows 7을 지원하는 최신 버전이 공개될 수 있습니다.                  |
| Linux    | N.A.            | -                                                                                                                            |

## 설정

대부분의 경우, Relay는 별 다른 설정을 하지 않더라도 곧바로 동작하도록 설계되어 있습니다. 하지만, 파일을 주고 받는데 어려움을 겪고 계신다면, Relay가 로컬 네트워크를 통해 데이터를 주고받을 수 있도록 방화벽 설정을 수정해야 할 수 있습니다.
...
앱이 (작업표시줄에) 숨겨진 상태로 실행하게 하려면, `--hidden` 플래그를 사용하세요 (예시: `relay.exe --hidden`).

v1.14.0보다 낮은 버전에서는, `autostart` 플래그가 설정되어 있고, 숨김 설정이 활성화되어 있을 때 앱이 숨겨진 상태로 실행됩니다.

## 작동 원리

Relay는 REST API 기술을 사용해 다른 기기와 안전하게 데이터를 주고받을 수 있는 통신 프로토콜을 이용합니다. 모든 데이터는 HTTPS를 통해 안전하게 전송되며, 각 기기마다 TLS/SSL 인증서가 생성되어 최대한의 안전성을 보장합니다.

Relay 프로토콜에 대해 더욱 자세한 정보를 알고 싶으시다면, [문서](https://github.com/relay/protocol)을 참고하시기 바랍니다.

## 시작하기

소스코드로부터 Relay를 컴파일하려면, 아래 과정들을 따라하시기 바랍니다:
...
3. `Relay` repository를 Clone하세요.
4. `cd app` 명령어를 통해 앱이 위치한 경로로 이동하세요.
5. `flutter pub get` 명령어를 통해 필요한 패키지를 다운로드 받으세요.
6. `flutter run` 명령어를 통해 앱을 실행하세요.

> [!중요]
> Relay는 현재 구 버전의 Flutter를 사용하고 있습니다. ([.fvmrc](.fvmrc) 참고)
> 따라서 시스템에 설치된 Flutter 버전과, Relay가 요구하는 버전이 맞지 않는데서 다양한 빌드 오류가 발생할 수 있습니다.
> 더욱 일관된 개발 환경을 확보하기 위해, Relay는 [fvm](https://fvm.app)을 사용하여 프로젝트 Flutter 버전을 관리하고 있습니다.
> `fvm`을 설치하신 뒤, `flutter` 명령어 대신 `fvm flutter` 명령어를 사용하시기 바랍니다.

## 기여하기

저희는 Relay를 개선하는데 관심을 갖고 있는 모든 분들로부터의 기여를 환영합니다. 만약 기여를 하고자 하신다면, 아래 몇 가지 방법들이 존재합니다:

### 번역

Relay를 다양한 언어로 번역하는데 도움을 주실 수 있습니다. 저희는 [Weblate](https://hosted.weblate.org/projects/relay/app) 플랫폼을 통해 번역을 관리하고 있습니다.

이 외에도, 해당 레포지토리를 포크하여 직접 번역문을 추가하는 방식으로도 기여하실 수 있습니다.

번역 파일은 [app/assets/i18n](https://github.com/relay/relay/tree/main/app/assets/i18n) 경로에서 찾아보실 수 있습니다. `_missing_translations_<locale>.json` 또는 `strings_<locale>.i18n.json` 파일을 수정하여 번역문을 추가하거나 개선할 수 있습니다.
...
- **개선:** Relay를 개선하기 위한 아이디어가 있으신가요? Issue를 생성하여 어떤 부분을 개선하면 좋을지 알려주세요.

더 많은 정보가 필요하시다면, [기여 가이드](https://github.com/relay/relay/blob/main/CONTRIBUTING.md)를 확인하시기 바랍니다.
