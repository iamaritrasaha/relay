...
Relay adalah aplikasi gratis dan sumber terbuka yang memungkinkan Anda berbagi file dan pesan dengan aman ke perangkat terdekat melalui jaringan lokal tanpa memerlukan koneksi internet.
...
Relay adalah aplikasi lintas platform yang memungkinkan komunikasi aman antar perangkat menggunakan REST API dan enkripsi HTTPS. Berbeda dengan aplikasi pesan lainnya yang mengandalkan server eksternal, Relay tidak memerlukan koneksi internet atau server pihak ketiga, menjadikannya solusi yang cepat dan andal untuk komunikasi lokal.
...
Dalam kebanyakan kasus, Relay seharusnya berfungsi langsung tanpa konfigurasi. Namun, jika Anda mengalami masalah dalam mengirim atau menerima file, Anda mungkin perlu mengonfigurasi firewall Anda untuk mengizinkan Relay berkomunikasi melalui jaringan lokal Anda.
...
Untuk memulai aplikasi dalam mode tersembunyi (hanya muncul di tray), gunakan flag --hidden (contoh: relay.exe --hidden).
...
Relay menggunakan protokol komunikasi yang aman, memungkinkan perangkat untuk berkomunikasi satu sama lain menggunakan REST API. Semua data dikirim dengan aman melalui HTTPS, dan sertifikat TLS/SSL dihasilkan secara langsung di setiap perangkat, memastikan keamanan maksimum.

Untuk informasi lebih lanjut tentang Protokol Relay, lihat [dokumentasi](https://github.com/relay/protocol).

## Memulai

Untuk mengompilasi Relay dari kode sumber, ikuti langkah-langkah berikut:

1. Instal Flutter [langsung](https://flutter.dev) atau menggunakan [fvm](https://fvm.app) (lihat [versi yang diperlukan](.fvmrc))
2. Clone repositori `Relay`
...
> [!CATATAN]
> Saat ini Relay memerlukan versi Flutter yang lebih lama (disebutkan dalam [.fvmrc](.fvmrc)) dan masalah build dapat terjadi karena ketidakcocokan antara versi Flutter yang diperlukan dan yang terinstal di sistem.
> Untuk membuat pengembangan lebih konsisten, Relay menggunakan [fvm](https://fvm.app) untuk mengelola versi Flutter proyek.
> Setelah menginstal `fvm`, jalankan `fvm flutter` alih-alih `flutter`.

## Berkontribusi

Kami menerima kontribusi dari siapa pun yang tertarik untuk membantu meningkatkan Relay. Jika Anda ingin berkontribusi, ada beberapa cara untuk terlibat:

### Terjemahan

Anda dapat membantu menerjemahkan Relay ke bahasa lain! **Cara yang direkomendasikan**: Gunakan platform [Weblate](https://hosted.weblate.org/projects/relay/app) untuk mengelola terjemahan.

**Alternatif**: Anda juga dapat berkontribusi dengan mem-fork repository ini dan menambahkan terjemahan secara manual.

Terjemahan terletak di direktori [app/assets/i18n](https://github.com/relay/relay/tree/main/app/assets/i18n). Edit file `_missing_translations_<locale>.json` atau `strings_<locale>.i18n.json` untuk menambah atau memperbarui terjemahan.

<a href="https://hosted.weblate.org/engage/relay/">
<img src="https://hosted.weblate.org/widget/relay/app/multi-auto.svg" alt="Status terjemahan" />
</a>
...
- **Peningkatan:** Punya ide untuk meningkatkan Relay? Silakan buat isu terlebih dahulu untuk mendiskusikan mengapa peningkatan tersebut diperlukan.

Untuk informasi lebih lanjut, lihat [panduan kontribusi](https://github.com/relay/relay/blob/main/CONTRIBUTING.md).
