...
Relay je bezplatná aplikace s otevřeným zdrojovým kódem, která vám umožňuje bezpečně sdílet soubory a zprávy s okolními zařízeními přes vaši místní síť, aniž byste potřebovali připojení k internetu.
...
Relay je multiplatformní aplikace, která umožňuje bezpečnou komunikaci mezi zařízeními pomocí REST API a šifrování HTTPS. Na rozdíl od jiných aplikací pro zasílání zpráv, které se spoléhají na externí servery, Relay nevyžaduje připojení k internetu ani servery třetích stran, což z něj činí rychlé a spolehlivé řešení pro místní komunikaci.
...
| Android   | 5.0             | -                                                                                                                                 |
| iOS       | 12.0            | -                                                                                                                                 |
| macOS     | 11 Big Sur      | Použijte OpenCore Legacy Patcher 2.0.2 (Více [#1005](https://github.com/relay/relay/issues/1005#issuecomment-2449899384)) |
...
Ve většině případů by měl Relay fungovat hned. Pokud však máte potíže s odesíláním nebo přijímáním souborů, možná budete muset nakonfigurovat bránu firewall tak, aby umožnila Relay komunikovat přes vaši místní síť.
...
Chcete-li aplikaci spustit skrytě (pouze v liště), použijte příznak `--hidden` (příklad: `relay.exe --hidden`).
...
Relay používá zabezpečený komunikační protokol, který umožňuje zařízením komunikovat mezi sebou pomocí REST API. Všechna data jsou odesílána bezpečně přes HTTPS a TLS/SSL certifikát je generován za chodu na každém zařízení, což zajišťuje maximální bezpečnost.

Další informace o protokolu Relay najdete v [dokumentaci](https://github.com/relay/protocol).

## Začínáme

Chcete-li zkompilovat Relay ze zdrojového kódu, postupujte následovně:
...
> Aby byl vývoj konzistentnější, používá Relay [fvm](https://fvm.app) ke správě verze projektu Flutter.
> Po instalaci `fvm` spusťte `fvm flutter` místo `flutter`.

## Jak přispět

Vítáme příspěvky od každého, kdo má zájem pomoci zlepšit Relay. Pokud chcete přispět, existuje několik způsobů, jak se zapojit:

### Překlady

Můžete pomoci s překladem Relay do jiných jazyků. Ke správě překladů používáme platformu [Weblate](https://hosted.weblate.org/projects/relay/app).

Případně můžete také přispět tím, že tento repozitář naklonujete a překlady přidáte ručně.

Překlady jsou umístěny v adresáři [app/assets/i18n](https://github.com/relay/relay/tree/main/app/assets/i18n). Chcete-li přidat nebo aktualizovat překlady, upravte soubor `_missing_translations_<locale>.json` nebo `strings_<locale>.i18n.json`.
...
- **Vylepšení:** Máte nápad, jak zlepšit Relay? Nejprve prosím vytvořte požadavek, abyste mohli prodiskutovat, proč je vylepšení potřeba.

Další informace najdete v [příručce pro přispívání](https://github.com/relay/relay/blob/main/CONTRIBUTING.md).
