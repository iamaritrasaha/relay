...
Relay — это кроссплатформенное приложение, которое обеспечивает безопасную связь между устройствами с помощью REST API и HTTPS-шифрования. В отличие от других приложений для обмена сообщениями, которые полагаются на внешние серверы, Relay не требует подключения к Интернету или сторонним серверам, что делает его быстрым и надежным решением для локальной связи.
...
[каналах дистрибуции]: https://github.com/relay/relay/blob/main/CONTRIBUTING.md#distribution
...
| Android   | 5.0                | -                                                                                                                                    |
| iOS       | 12.0               | -                                                                                                                                    |
| macOS     | 11 Big Sur         | Используйте OpenCore Legacy Patcher 2.0.2 (См. [#1005](https://github.com/relay/relay/issues/1005#issuecomment-2449899384))  |
...
В большинстве случаев Relay должен работать прямо из коробки. Однако, если у вас возникли проблемы с отправкой или получением файлов, возможно, вам придется настроить брандмауэр, чтобы позволить Relay взаимодействовать с вашей локальной сетью.
...
Чтобы запустить приложение скрытно (только в трее), используйте флаг `--hidden` (например: `relay.exe --hidden`).
...
Relay использует безопасный протокол связи, который позволяет устройствам взаимодействовать между собой с помощью REST API. Все данные безопасно передаются через HTTPS протокол, а сертификат TLS/SSL генерируется на лету на каждом устройстве, обеспечивая максимальную безопасность.

Для получения дополнительной информации о протоколе Relay смотрите [документацию](https://github.com/relay/protocol).

## Начало работы

Чтобы скомпилировать Relay из исходного кода, выполните следующие действия:
...
3. Клонируйте `Relay` репозиторий
...
> [!NOTE]
> На данный момент Relay требует более старой версии Flutter (указанной в [.fvmrc](.fvmrc)),
...
> Чтобы сделать разработку более согласованной, Relay использует [fvm](https://fvm.app) для управления версией проекта Flutter.
> После установки `fvm`, запустите `fvm flutter` вместо `flutter`.

## Вклад в развитие проекта

Мы приветствуем вклады от всех, кто хочет помочь улучшить Relay. Если вы хотите внести вклад, есть несколько способов принять участие:

### Перевод

Вы можете помочь перевести Relay на другие языки. **Рекомендуемый способ**: Используйте платформу [Weblate](https://hosted.weblate.org/projects/relay/app) для управления переводами.

**Альтернативный способ**: Вы также можете внести свой вклад, создав ответвление этого репозитория и добавив переводы вручную.

Переводы находятся в директории [app/assets/i18n](https://github.com/relay/relay/tree/main/app/assets/i18n). Отредактируйте файл `_missing_translations_<locale>.json` или файл `strings_<locale>.i18n.json`, чтобы добавить или обновить переводы.

<a href="https://hosted.weblate.org/engage/relay/">
<img src="https://hosted.weblate.org/widget/relay/app/multi-auto.svg" alt="Статус перевода" />
</a>
...
- **Улучшение:** Есть идея, как улучшить Relay? Пожалуйста, сначала создайте соответствующую тему, чтобы обсудить, почему это улучшение необходимо.

Для получения дополнительной информации смотрите руководство о [вкладе в развитие проекта](https://github.com/relay/relay/blob/main/CONTRIBUTING.md).
